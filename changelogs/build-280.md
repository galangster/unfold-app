...
Everything in 1.1.8 (build 279), plus the client half of the signed device credential. Every request to the Unfold backend now carries a per-device credential minted by the server. A request the server rejects for a stale credential re-registers once and retries. There is no visible change; this build proves nothing broke.
...
- **Every backend call heals** - Sync, generation, companion, examen, scripture explain, stories, audio, bug reports, voice check-ins, recommendations, and journal Go Deeper all use the same self-healing request path. A lint rule now blocks any new backend call that skips it.
