# Build 87 - Editor strip-down to spike parity
...
- `UnfoldEditorController` trimmed from 1158 to 796 lines. Removed: `shouldHandle` Enter intercept, textDidChange backup observer, checklist tap gesture, `ensureCaretVisible`, `extendBlockMarkers`, `textChangeGeneration` guard. All commands (bold/italic/underline/strike, H1-H3, lists, indent/outdent, link, scripture, image, undo/redo) are unchanged.
- Vendored Proton guard change: `textView.delegate === self` to `textView is RichTextView` in 5 sites across `RichTextViewContext` and `RichTextEditorContext`.

## What to Test

- [ ] Start a numbered list, type a line, press Enter, confirm `2.` appears, type another line, Enter, confirm `3.` appears
