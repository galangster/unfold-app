...
Usage: render-clips.py [outdir] [scene ...]"""
...
OUT=sys.argv[1] if len(sys.argv)>1 else os.path.join(ROOT,"harness","clips"); ONLY=set(sys.argv[2:])
...
jobs=[(d,os.path.basename(d).replace("today-",""),m,i) for d in scenes if not ONLY or os.path.basename(d).replace("today-","") in ONLY for m in THEMES for i in range(600)]
...
for clip in sorted(os.listdir(os.path.join(OUT,"frames"))):
    if ONLY and clip.rsplit("-",1)[0] not in ONLY: continue
