...
| 2 Static audit (12 dims) | VERIFY RESUME RUNNING | Run `wf_27087d05-836` (resume 3): finders+dedupe DONE — **152 found → 141 canonical**; taste judged (8 kept). Verification: 16 confirmed before spend-cap trip #3 (COR-1..11 incl. 4 P1s: streak-never-resets, freeze-burn-on-free-gap, journal TDZ, plus PERF-1..5ish); ~330 verifier lenses + ledger re-running from cache-resume. CAUTION: interim "refuted=116" was poisoned by limit-failures (unverified ≠ refuted) — final resume recomputes. Finder files: `/tmp/unfold-e2e-audit-2026-06/findings/*.md`. |
