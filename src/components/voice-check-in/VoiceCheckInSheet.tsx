...
function buildWaveform(level: number, step: number): number[] {
...
export function VoiceCheckInSheet({
  ...
  const renderRecording = () => (
    <Animated.View entering={reducedMotion ? undefined : FadeIn.duration(180)} style={styles.stateContent}>
      ...
      <Text accessibilityRole="timer" style={[styles.timer, { color: colors.text }]}>{formatTime(activeDurationMs)}</Text>
      <View
        accessible
        accessibilityRole="progressbar"
        accessibilityLabel={`Recording ${formatTime(activeDurationMs)}`}
  ...
  const renderReview = () => {
    ...
    return (
      <Animated.View entering={reducedMotion ? undefined : FadeIn.duration(180)} style={styles.stateContent}>
        ...
        <Text style={[styles.timer, { color: colors.text }]}>{formatTime(isPlaying ? playbackMs : recordedDurationMs)}</Text>
        <Pressable
          accessibilityRole="button"
        ...
        >
          ...
          <Text style={[styles.playbackTime, { color: colors.textMuted }]}>{formatTime(recordedDurationMs)}</Text>
        </Pressable>
