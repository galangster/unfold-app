...
import { recordAnnouncement } from '@/lib/feature-announcements';
...
export function MusicAnnouncement({
  visible,
  actionLabel,
  previewAllowed,
  onClose,
  ...
  }, [fail, previewAllowed, status, stopPreview]);

  const close = (outcome: 'dismissed' | 'tried' = 'dismissed') => {
    stopPreview();
    recordAnnouncement(MUSIC_ANNOUNCEMENT.id, outcome);
    onClose();
  };
      ...
      <Pressable accessible={false} style={styles.backdrop} onPress={() => close()}>
        ...
        >
          <Pressable
            ...
            accessibilityLabel="Close music announcement"
            onPress={() => close()}
            style={styles.close}
          >
          ...
          >
            {MUSIC_ANNOUNCEMENT.title}
          </AmbientText>
          <AmbientText style={[styles.body, { color: colors.textMuted }]}>
            {MUSIC_ANNOUNCEMENT.description}
          </AmbientText>
          <View style={[styles.taste, { borderColor: colors.border }]}>
            ...
            <View style={styles.tasteControls}>
              ...
              <View style={styles.waveform} accessibilityElementsHidden>
                {MUSIC_ANNOUNCEMENT.waveform.map((height, index) => {
                  const heard = index / MUSIC_ANNOUNCEMENT.waveform.length < progress;
                  return (
                    <View
                      key={`${height}-${index}`}
                      style={{
          ...
          <Pressable
            ...
            onPress={() => {
              stopPreview();
              if (onTry(MUSIC_ANNOUNCEMENT.songId)) close('tried');
            }}
          ...
          <Pressable accessibilityRole="button" onPress={() => close()} style={styles.later}>
            <AmbientText style={[styles.action, { color: colors.text }]}>
              Maybe later
            </AmbientText>
          </Pressable>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  tasteTop: { flexDirection: 'row', justifyContent: 'space-between', gap: 12 },
  ...
  tasteStatus: { flexDirection: 'row', justifyContent: 'space-between' },
  ...
  action: { fontFamily: FontFamily.ui, fontSize: 15 },
});
