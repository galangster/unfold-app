...
export function DevotionalWebView({
  ...
  // Build a re-apply script from the current highlights — fired on WebView load/reload
  // via the onLoadEnd callback below. No useEffect needed — WebView calls us when ready.
  const handleLoadEnd = useCallback(() => {
    if (!webViewRef.current || existingHighlights.length === 0) return;
    const ranges = existingHighlights
      .filter((h) => h.serializedRange)
    ...
    webViewRef.current.injectJavaScript(`
      (function apply() {
        ...
        try { window.rangyHighlighter.removeAllHighlights(); } catch (e) {}
        try {
          var typeHeader = 'type:textContent';
          var allRanges = ${JSON.stringify(ranges)};
          var seen = {};
          var parts = [];
          allRanges.forEach(function(r) {
            r.split('|').forEach(function(p) {
              if (p && !p.startsWith('type:') && !seen[p]) { seen[p] = true; parts.push(p); }
            });
          });
          if (parts.length > 0) {
            window.rangyHighlighter.deserialize(typeHeader + '|' + parts.join('|'));
          }
        } catch (e) {}
      })();
      true;
  ...
  const injectedJavaScript = useMemo(() => {
    console.log('[Highlight] WebView rebuilding injected JS', { existingCount: existingHighlights.length });
    ...
    console.log('[Highlight] WebView serialized ranges:', allSerializedRanges.length);
    ...
    return `
      ...
      function handleColorTap(btn, e) {
        ...
        window.ReactNativeWebView.postMessage(JSON.stringify({
          type: 'DEBUG_LOG',
          message: '[Highlight WebView] handleColorTap START',
        ...
        window.ReactNativeWebView.postMessage(JSON.stringify({
          type: 'DEBUG_LOG',
          message: '[Highlight WebView] snapshot check',
          hasSnapText: !!snapText,
          snapTextLen: snapText ? snapText.length : 0,
          hasRangyHighlighter: !!window.rangyHighlighter,
        }));

        if (!snapText || !window.rangyHighlighter) {
          window.ReactNativeWebView.postMessage(JSON.stringify({
            type: 'DEBUG_LOG',
            message: '[Highlight WebView] BAILING — no snapText or no rangyHighlighter',
          }));
          return;
        }
...
<head>
  ...
  <!-- Rangy for robust text highlighting — load from CDN as primary, inlined as fallback -->
  ...
  const handleMessage = (event: any) => {
    try {
      ...
      if (data.type === 'QUOTE_SELECTED' && onQuoteSelected) {
        console.log('[Highlight] WebView → QUOTE_SELECTED', { text: data.text?.substring(0, 40), color: data.color, hasSerializedRange: !!data.serializedRange });
        onQuoteSelected({
          text: data.text,
      ...
      } else if (data.type === 'QUOTE_SELECTED' && !onQuoteSelected) {
        console.log('[Highlight] WebView → QUOTE_SELECTED but no onQuoteSelected handler');
