...
describe('getEveningWindDownBody', () => {
  it('leads with the day\'s act, the thing the devotional asked the reader to do', () => {
    const body = getEveningWindDownBody({ title: 'The Unfinished House', act: LONG_ACT, eveningScriptureRef: 'Psalm 132:1-5' });
  ...
  it('falls back to the evening scripture, then to a template that names the day', () => {
    expect(getEveningWindDownBody({ title: 'Rest', eveningScriptureRef: 'Psalm 4:8' })).toBe(
      'Before rest, sit with Psalm 4:8 for a minute.',
    );
    const templated = getEveningWindDownBody({ title: 'Rest', scriptureReference: 'Psalm 4' });
    expect(templated).toMatch(/Rest|Psalm 4/);
  });

  it('uses the generic pool when nothing about the day is known', () => {
    expect(EVENING_MESSAGES).toContain(getEveningWindDownBody(null));
  });
});
...
describe('getMiddayCheckInBody', () => {
  ...
  it('then uses the day\'s own check-in question', () => {
    expect(getMiddayCheckInBody({ checkInQuestion: 'Where did you notice grace?' }, null)).toBe(
      'Where did you notice grace?',
    );
  });

  it('uses the generic pool when nothing about the day is known', () => {
    expect(MIDDAY_MESSAGES).toContain(getMiddayCheckInBody(null, null));
  });
});
