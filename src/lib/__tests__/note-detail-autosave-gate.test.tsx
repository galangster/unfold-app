/**
 * The premium creation gate ran on every keystroke: scheduleAutoSave called
 * gate() and returned early when it said no. The gate answers 'blocked' while
 * the premium policy is still 'unknown' (RevenueCat has not answered yet) —
 * so typing during those seconds scheduled no save at all and, on a churned
 * account, each keystroke also tried to raise a paywall. Gating belongs on
 * the actions that CREATE or OPEN a note, never on persisting words already
 * typed into an open editor. This drives the REAL NoteDetailScreen.
 */
...
(globalThis as any).__getHtmlImpl = async () => '';
(globalThis as any).__gate = () => true;
(globalThis as any).__gatePolicy = 'granted';
...
jest.mock('@10play/tentap-editor', () => {
  ...
  return {
    ...
    // Keyboard up: the editing chrome (Minimize/Done) follows keyboard visibility.
    useKeyboard: () => ({ keyboardHeight: 300, isKeyboardUp: true }),
    RichText: () => null,
    TenTapStartKit: [],
    TaskListBridge: {},
    HeadingBridge: {},
...
jest.mock('@/hooks/useCreationGate', () => ({
  useCreationGate: () => ({
    policy: (globalThis as any).__gatePolicy,
    isPremium: true,
    gate: () => (globalThis as any).__gate(),
...
const TYPED = '<p>a sentence typed while the subscription check was still pending</p>';
...
describe('note autosave vs the premium creation gate', () => {
  ...
  beforeEach(() => {
    ...
    (globalThis as any).__getHtmlImpl = async () => '';
    (globalThis as any).__noteParams = { noteId: 'note-1' };
    // The policy is still unknown: the gate blocks every creation action.
    (globalThis as any).__gatePolicy = 'unknown';
    (globalThis as any).__gate = () => { gateCalls += 1; return false; };
  ...
  it('saves typing in an already-open note while the gate is still blocking', () => {
    let tree: any;
    act(() => { tree = renderer.create(<NoteDetailScreen />); });

    act(() => { editorNode(tree).props.onChangeHtml({ nativeEvent: { html: TYPED } }); });
    act(() => { jest.advanceTimersByTime(AUTOSAVE_DEBOUNCE_MS + 50); });

    expect(useUnfoldStore.getState().notes[0].content).toBe(TYPED);
    // …and the save path never consults the gate (each call would also try to
    // raise a paywall or an alert on a churned account).
    expect(gateCalls).toBe(0);
    act(() => tree.unmount());
  });

  it('saves a title edit under the same conditions', () => {
    let tree: any;
    act(() => { tree = renderer.create(<NoteDetailScreen />); });
    ...
    act(() => { titleInput.props.onChangeText('Renamed mid-check'); });
    act(() => { jest.advanceTimersByTime(AUTOSAVE_DEBOUNCE_MS + 50); });

    expect(useUnfoldStore.getState().notes[0].title).toBe('Renamed mid-check');
    expect(gateCalls).toBe(0);
    act(() => tree.unmount());
  });

  it('still gates the actions that create something new', () => {
    let tree: any;
    act(() => { tree = renderer.create(<NoteDetailScreen />); });

    // Minimize (which persists a draft and docks it) is a user action, not a
    // keystroke: it must still ask.
    const minimize = tree.root.findAll(
      (node: any) => node.props.accessibilityLabel === 'Minimize note' && typeof node.type !== 'string',
    )[0];
